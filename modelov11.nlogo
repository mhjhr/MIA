...
to setup-cars
  ...
  create-turtles numero-de-camiones-flota1 [
    ...
    set ready? false
    set preparation_time (tiempo_preparacion_minutos + tiempo_preparacion_minutos * ((random 10) / 100) ) * 4 ; 1 minuto = 4 ticks
...
to go
    ...
    ;; Trucks to lunch
     ...
     if comer? = true [
      ...
      if [ pxcor ] of patch-here = -12 and [ pycor ] of patch-here = -1 and heading = 270 [
        ...
        set time_finish_lunch actual-time + 240 + (random delta_almuerzo_minutos) * 4
  ...
  ask patches with [pycor = 0 and pxcor = 20 ] [
    ask turtles-here with [ready? = true][
     ...
     if downloading = 0 [
     set speed 0
     set last-wait-time-descarga (tiempo-min-de-descarga + random (tiempo-max-de-descarga - tiempo-min-de-descarga))/ Num_frentes_zona_norte
  ...
  ask patches with [pycor = -1 and pxcor = -20] [
    ask turtles-here with [flota = 1 and ready? = true][
      ...
      if time_box < 11 and turno_finalizado? = false [
        ...
        if uploading = 0 [
          set speed 0
          set last-wait-time-carga (tiempo-min-de-carga + random (tiempo-max-de-carga - tiempo-min-de-carga))/ Num_Escavadoras
      ...
      if time_box >= hora_inicio_fin_turno and turno_finalizado? != true [
  ...
  ask patches with [pycor = -40 and pxcor = 29] [
    ask turtles-here with [flota = 2 and ready? = true][
     ...
     if uploading = 0 [
     set speed 0
     set last-wait-time-carga (tiempo-min-de-carga + random (tiempo-max-de-carga - tiempo-min-de-carga)) / Num_frentes_zona_sur
     set wait-time-carga actual-time + last-wait-time-carga
     set uploading 1
    ]
      if wait-time-carga > actual-time [
